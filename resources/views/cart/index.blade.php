...
@section('content')
                                        ...
                                        @foreach (Cart::content() as $product)
                                            <tr>
                                                ...
                                                <td class="border-0 align-middle">
                                                    <strong>{{ $product->model->getPrice() }}</strong>
                                                </td>
                                                <td class="border-0 align-middle">
                                                ...
                                                <td class="border-0 align-middle">
                                                    <form action="{{ route('cart.destroy', $product->rowId) }}"
                                                        ...
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-dark btn btn-danger"><i
                            ...
                            <div class="p-4">
                                ...
                                <ul class="list-unstyled mb-4">
                                    <li class="d-flex justify-content-between py-3 border-bottom"><strong
                                            class="text-muted"> Subtotal </strong><strong>{{getPrice(Cart::subtotal())}}</strong></li>
                                    {{-- <li class="d-flex justify-content-between py-3 border-bottom"><strong
                                ...
                                </ul><a href="{{route ('checkout.index')}}" class="btn btn-dark rounded-pill py-2 btn-block">Procceed to
                                    checkout</a>
                            </div>
                        </div>
                    </div>
    ...
    @else
        <div class="col-md-12">
            <p> <strong>Votrre panier est vide</strong> </p>
    ...
    <script>
        ...
        Array.from(selects).forEach((element) => {
          element.addEventListener('change',function () {
            ...
            var token = document.querySelect('meta [name="csrf-token"]').getAttribute('content');
            fetch(
                `/panier/${rowId}`,
                {
                    herders:
